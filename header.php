			...
			<div id="header">
				<div id="header-inner">
					...
					<div class="header-logo-block">
						...
						$clientLogoRetina = COption::GetOptionInt("bitrix24", "client_logo_retina", "");
						...
						if (file_exists($_SERVER["DOCUMENT_ROOT"].SITE_DIR."include/company_name.php") && !$clientLogo && !$clientLogoRetina && !$siteTitle)
						{
						...
						else
						{
							...
							<a id="logo_24_a" href="<?=htmlspecialcharsbx(SITE_DIR)?>" title="<?=GetMessage("BITRIX24_LOGO_TOOLTIP")?>" class="logo"><?
								...
								?>
								<span id="logo_24_text" <?if ($clientLogo || $clientLogoRetina):?>style="display:none"<?endif?>>
								...
								<span class="logo-img-span">
									<img id="logo_24_img"
										 <?if ($clientLogo):?>src="<?=CFile::GetPath($clientLogo)?>"<?endif?>
										 <?if (!$clientLogo && !$clientLogoRetina):?>style="display:none;"<?endif?>
										 <?if ($clientLogoRetina):?>srcset="<?=CFile::GetPath($clientLogoRetina)?> 2x"<?endif?>
									/>
								</span>
								<?
					...
					?>
					<?$APPLICATION->IncludeComponent("bitrix:search.title", (COption::GetOptionString("intranet", "search_title_old", "") == "Y" ? ".default_old" : ""), array_merge(array(
						"CHECK_DATES" => "N",
...
{
	$APPLICATION->IncludeComponent("vportal:vivi_notify", "",
		Array(
			"BLOG_URL" => "",
